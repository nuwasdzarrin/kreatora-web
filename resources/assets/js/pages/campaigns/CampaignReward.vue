    ...
    <div class="container d-flex mt-5 reward-horizontal">
      <div class="mr-3" v-for="(item, index) in detail_campaign.rewards" :key="index">
        <div class="card-reward-content">
          ...
          <button class="btn btn-primary btn-block" :disabled="remainingBacker(item) == 0" @click="$router.push({ name: 'CampaignSupport', params: { slug: detail_campaign.slug }, query: { reward: item.id }})">Pilih</button>
        </div>
      </div>
    </div>
